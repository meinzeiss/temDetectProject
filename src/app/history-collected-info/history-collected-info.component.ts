...
@Component({
  selector: 'app-history-collected-info',
  templateUrl: './history-collected-info.component.html',
  styleUrls: ['./history-collected-info.component.css']
})

export class HistoryCollectedInfoComponent implements OnInit {
  interTime: any;
  ...
  initDefaultCalcShowList() {
    ...
    for (let i = 0; i < 15; i++) {
      let chnNm = `CH0${Math.floor(i / 5) + 1}0${Math.floor(i % 5) + 1}`;
    ...
    if (canCalc) {
      ...
      if (this.rangeLeft.length > 0 && this.rangeRight.length > 0
        ...
        if (taskUid != null && taskUid != undefined && taskUid.length > 0) {
          let sensors: string[] = [];
          for (let x = 0; x < 15; x++) {
            let tmpStr = `Sensor${Math.floor(x / 5)}${x % 5}`;
  ...
  */
  ...
  // changeShowView() {
  //   if (this.chartShowFlg) {
  //     this.initDefaultCalcShowList();
  //   }
  //   let tmpSelected:string ='';
  //   this.chartShowFlg = !this.chartShowFlg;
  //   console.log('changeShowView on Called');
  //   console.log('chartShowFlg = ', this.chartShowFlg);
  //   if(this.chartShowFlg){
  //     console.log('init chart selected on Called');
  //     tmpSelected = '{';
  //     for (let i = 0; i < 15; i++) {
  ...
  // }
  // onTasksSearch() {
  //   let startStr = this.formatTime(this.dateRange[0]);
  //   let endStr = this.formatTime(this.dateRange[1]);
  //   this.message = 'start date = ' + startStr + ', end date = ' + endStr;
  //   console.log(this.message);
  //   let queryParams = new HttpParams()
  //     .append("PageIndex", 0)
  //     .append("StartTime", startStr)
  //     .append("EndTime", endStr)
  //     .append("CountPerPage", 10);
  //   console.log(queryParams);
  //   this.http.get(env.url(env.Api_HistoryTasks), { params: queryParams }).subscribe({
  //     complete: () => { }, // completeHandler
  //     error: (e) => {
  ...
  //       // console.log(res);
  //       // console.log(res.RtnData.Data);
  //       if (res != null && res != undefined && res.Code == 0
  //         && res.RtnData != null && res.RtnData != undefined
  //         && res.RtnData.Data != null && res.RtnData.Data != undefined && res.RtnData.Data.length > 0) {
  //         let arrayData = res.RtnData.Data;
  //         let size = arrayData.length;
  //         this.listOfData = [];
  //         console.log(arrayData);
  //         for (let i = 0; i < arrayData.length; i++) {
  //           let startDt = new Date(arrayData[i].ITime);
  //           let endDt = new Date(arrayData[i].EndTime);
  //           let dTime = (endDt.getTime() - startDt.getTime()) / (60 * 1000);
  //           let uid = arrayData[i].Uid;
  //           let status = arrayData[i].Status;
  //           // console.log(`Uid = ${uid}, Status: ${status}`);
  //           // console.log(`start: ${startDt.toISOString()}, end: ${endDt.toISOString()}, 持续：${dTime.toFixed()} 分钟`);
  //           let task: Task = {
  //             Uid: uid,
  //             ITime: arrayData[i].ITime,
  //             EndTime: arrayData[i].EndTime,
  //             Status: arrayData[i].Status,
  //             // Duration: dTime.toFixed(),
